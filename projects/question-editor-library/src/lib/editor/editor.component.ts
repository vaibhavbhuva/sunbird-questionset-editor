...
export class EditorComponent implements OnInit, AfterViewInit {
  ...
  fetchQuestionSetHierarchy() {
    this.editorService.getQuestionSetHierarchy('do_122345').pipe(catchError(error => {
      const errInfo = {
  ...
  submitCollection() {
    this.editorService.sendQuestionSetForReview('do_1234567').pipe(catchError(error => {
      const errInfo = {
